...
std::expected<Table, std::string> SQLiteConnector::ExecuteQuery(const std::string &query) {
    ...
    if (ret_code != SQLITE_OK) {
        ...
        return std::unexpected(std::format("Ошибка запроса в SQLiteConnector::ExecuteQuery {}:{} {}", loc.line(),
    ...
    if (ret_code != SQLITE_DONE) {
        std::string error_message = sqlite3_errmsg(db);
        constexpr std::source_location loc = std::source_location::current();
        return std::unexpected(std::format("Ошибка завершения запроса в SQLiteConnector::ExecuteQuery {}:{} {}",
...
std::expected<bool, std::string> SQLiteConnector::ExecuteCommand(const std::string &query) const {
    ...
    if (ret_code != SQLITE_OK) {
        std::string error_message = sqlite3_errmsg(db);
        constexpr std::source_location loc = std::source_location::current();
        return std::unexpected(std::format("Ошибка запроса в SQLiteConnector::ExecuteCommand {}:{} {}", loc.line(),
                                           loc.column(), error_message));
    }
    return true;
}
...
std::expected<Row, std::string> SQLiteConnector::GetTableList() {
    ...
    if (return_code != SQLITE_OK) {
        std::string error_message = sqlite3_errmsg(db);
        constexpr std::source_location loc = std::source_location::current();
        return std::unexpected(std::format("Ошибка списка таблиц SQLiteConnector::GetTableList {}:{} {}", loc.line(),
                                           loc.column(), error_message));
    }
    ...
    if (return_code != SQLITE_DONE) {
        std::string error_message = sqlite3_errmsg(db);
        constexpr std::source_location loc = std::source_location::current();
        return std::unexpected(std::format("Ошибка завершения запроса в SQLiteConnector::GetTableList {}:{} {}",
                                           loc.line(), loc.column(), error_message));
    }
    sqlite3_finalize(stmt);
    return result;
...
std::expected<Row, std::string> SQLiteConnector::GetTableSchema(const std::string &tableName) {
    ...
    if (ret_code != SQLITE_OK) {
        ...
        return std::unexpected(
            std::format("Ошибка получения схемы таблицы {} в SQLiteConnector::GetTableSchema {}:{} {}", tableName,
                        loc.line(), loc.column(), error_message));
    }
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        ...
        auto loc = std::source_location::current();
        sqlite3_finalize(stmt);
        return std::unexpected(std::format("Ошибка завершения запроса в SQLiteConnector::GetTableSchema {}:{} {}",
                                           loc.line(), loc.column(), error_message));
    }
